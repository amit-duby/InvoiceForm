...
const InvoiceForm = () => {
  ...
  const debouncedValidateEmail1 = useCallback(
    debounce((email1) => {
      setIsValid(validateEmail1(email1));
    }, 300),
    []
  );
  const handleBill1Change = (e) => {
    const newEmail = e.target.value;
    setEmail1(newEmail);
    debouncedValidateEmail1(newEmail);
  };
  // demakl22
  ...
  const validateInvoice = () => {
    if (!invoiceNo) {
      setInvoiceError("InvoiceNo require");
      return false;
    }
    setInvoiceError("");
    return true;
  ...
  const handleClIEmail = (e) => {
    e.preventDefault();
    setCliEmail(true);
  };

  const handleCLIPhone = (e) => {
    e.preventDefault();
    setCliPhone(true);
  };

  const handleCLIGst = (e) => {
    e.preventDefault();
    setCliGst(true);
  };

  const handleCLIPan = (e) => {
    e.preventDefault();
    setCliPan(true);
  };
  ...
  //  billbyphon
  const validateByPhoneNumber = (phoneNumber) => {
    if (!phoneNumber) {
      setError("");
      return;
    }
  ...
  const handleByPhoneChange = (value) => {
    setBillByPhone(value);
    validateByPhoneNumber(value);
  };

  useEffect(() => {
    if (isPhone && Billbyphone) {
      validateByPhoneNumber(Billbyphone);
    }
  }, [isPhone, Billbyphone]);
  // billto phone
  ...
  useEffect(() => {
    if (cliPhone && Billtophone) {
      validatePhoneNumber(Billtophone);
    }
  }, [cliPhone, Billtophone]);
  ...
  const handleRemoveBillClick = (e) => {
    e.preventDefault();
    setSubmitValue(submitValue.remove);
  };
  const handleRemoveBillClicks = (e) => {
    e.preventDefault();
  ...
  const handleAddGSTInfoRemoveClick = (id) => {
    setGSTInputSets(gstinputSets.filter((inputSet) => inputSet.id !== id));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (data) {
      ...
      // console.log(JSON.stringify(parsedData.user), "user");
    ...
    if (!signatureImage) {
      alert("first upload signature after update");
    }
    if (!image) {
      alert("first upload image after update");
    }

    if (!document) {
      alert("first upload documentimage after update");
    }
    ...
    const isInvoiceNoValid = validateInvoice(invoiceNo);
    const isInvoiceDateValid = validateInvoiceDate(invoiceDate);
    const isDueDateValid = validateDueDate(dueDate);
    // const isBillToPhoneValid = validatePhoneNumber(Billtophone);
    // const isBillByPhoneValid = validateByPhoneNumber(Billbyphone);
    if (isDueDateValid && isInvoiceDateValid && isInvoiceNoValid) {
      ...
      try {
        ...
        console.log(response, "dinvoid");
        alert("Invoice data created successfully!");

        setIsLoading(false);

        setImage({ url: response.data.url });
        setDocument({ url: response.data.url });

        navigate(`/dashboard/user/getInvoiceByUserId/${userId}`);
      } catch (error) {
        console.error(
          "Error uploading invoice data:",
    ...
    <>
      <div className="kjnpBp gAqflM dLJlQK dyGQst">
        <div className="css-1jx43s9 formd">
          <form className="">
            ...
            <div className="bOoGEV">
              <div className="lkWYBX">
                ...
                {invoiceDateError && (
                  <p style={{ color: "red" }}>{invoiceDateError}</p>
                )}
                {due && (
                  <div className="css-offwra cfOcjU">
                    ...
                    <input
                      ...
                      name="dueDate"
                      className="inptS"
                      style={{ maxWidth: "30em", marginLeft: "2em" }}
                      value={dueDate}
                      ...
                      <div className="flex ">
                        <div className=" flex mr-10">
                          <input
                            ...
                            onChange={(event) =>
                              handleChangeClick(index, "firstName", event)
                            }
                          />
                          <input
                            type="text"
                            ...
                            onChange={(event) =>
                              handleChangeClick(index, "value", event)
                            }
                          />
                        </div>
                        <button
                    ...
                    <div>
                      {isPan && (
                        <input
                          type="text"
                          name="Billbypan"
                          value={user.Billbypan}
                          onChange={handleChange}
                          placeholder="Your PAN (optional)"
                          className="inptS lg:mr-3"
                          style={{
                            width: "100%",
                            maxWidth: "12em",
                            // marginLeft: "1em",
                  ...
                  <div key={inputSet.id} className="my-4">
                    <div className="lg:flex lg:mb-0">
                      ...
                      >
                        ...
                        />
                        <button
                          className="remove-btn text-xl mb-2 lg:mb-0"
                          onClick={() =>
          ...
          {/* <AddSomemorefield/> */}
          <div>
            <div className="lg:flex lg:gap-4  sm:mt-70 ml-3 ">
              {!term && (
                <div>
                  <div
                    className="bg-violet-50 "
